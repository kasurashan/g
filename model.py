...
class FCOSHead(nn.Module):
    def __init__(self, in_channel, n_class, n_conv, prior):
        ...
        self.center_pred = nn.Conv2d(in_channel, 1, 3, padding=1) #####################no centerness
    ...
    def forward(self, input):
        ...
        return logits, bboxes, centers ############## no centerness
...
class FCOS(nn.Module):
    def __init__(self, config, backbone):
        ...
        self.loss = FCOSLoss(
            ...
            config.center_sample, #########no centerness
            config.fpn_strides,
            config.pos_radius,
        )
    ...
    def forward(self, input, image_sizes=None, targets=None):
        ...
        cls_pred, box_pred, center_pred = self.head(features) ########no centerness

        #print("asdflossssss", type(center_pred), len(center_pred) , center_pred)
