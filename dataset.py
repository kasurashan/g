...
class COCODataset(datasets.CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        key_custom = [o['keypoint'] for o in annot] ######################## i add this for keypoint

        #print("!@#$%^^^^^^^^^^^^^^^^bb", type(key_custom), len(key_custom), key_custom)

        boxes = torch.as_tensor(boxes).reshape(-1, 4)
        key_custom = torch.as_tensor(key_custom).reshape(-1, 2) ####################i add this for keypoint

        #target = BoxList(boxes, img.size, mode='xywh').convert('xyxy') #######i edit this
        target = BoxList(boxes, key_custom, img.size, mode='xywh').convert('xyxy')
        #print("^^^^^^^^^^^", target.key)
