...
class FCOSPostprocessor(nn.Module):
    ...
    def forward_single_feature_map( 
        ...
        #print("centerasdfasdfasdf", center_pred, type(center_pred), center_pred.shape)
        ...
        candid_ids = candid_ids.contiguous()   ############### for contiguous i add it
        top_ns = candid_ids.view(batch, -1).sum(1)
        top_ns = top_ns.clamp(max=self.top_n)

        #print("afsdasdf", center_pred[:, :, None], center_pred[:, :, None].shape, type(center_pred[:, :, None]) )
        ...
        for i in range(batch):
            ...
            print(center_pred.shape,type(center_pred) ,"asdf", detections.shape, type(detections))
            #boxlist = BoxList(detections, (int(width), int(height)), mode='xyxy')
            boxlist = BoxList(detections, center_pred, (int(width), int(height)), mode='xyxy') ############# edit error
...
################### instead of center_pred, we use "keypoint" !!!!!
    def forward_single_feature_map(
        self, location, cls_pred, box_pred, center_pred, image_sizes
    ):
    ...
    def forward(self, location, cls_pred, box_pred, image_sizes):
        boxes = []

        for loc, cls_p, box_p in zip(
            location, cls_pred, box_pred
        ):
            boxes.append(
                self.forward_single_feature_map(
                    loc, cls_p, box_p, image_sizes
                )
            )
