...
class BoxList:
    #def __init__(self, box, image_size, mode='xyxy'):   ######### i edit this for keypoint
    def __init__(self, box, key, image_size, mode='xyxy'):
        device = box.device if hasattr(box, 'device') else 'cpu'
        
        #print(type(key), type(box),"wwwwwwwwwwwwwww", type(image_size), type(mode))
        box = torch.as_tensor(box, dtype=torch.float32, device=device)
        #print("@@@@@@@@@@", type(box))
        key = torch.as_tensor(key, dtype=torch.float32, device=device) ############
        #print("@$$$$$$$$", type(key))
        
        #print("!@#$%^^^^^^^^^^^^^^^^bb", type(key), len(key), key)


        self.box = box
        self.key = key   ##########i add this for keypoint
    ...
    def convert(self, mode):
        ...
        if mode == 'xyxy':
            ...
            box = BoxList(box, self.key, self.size, mode=mode)############i edit this
    ...
    def __getitem__(self, index):
        ...
        #print("GETITEMgetitenm!@@#$!", self.fields)
    ...
    def resize(self, size, *args, **kwargs):
        ...
        #print("@@@RESIZEEEEEEEaa", self.box)
        if ratios[0] == ratios[1]:
            ratio = ratios[0]
            scaled = self.box * ratio
            scaled2 = self.key * ratio   #########i addd this for keypoint
            #box = BoxList(scaled, size, mode=self.mode)
            box = BoxList(scaled, scaled2, size, mode=self.mode) #########################
            #print("$$$$RESIZEEEEEEEaa", box.box)
            for k, v in self.fields.items():
        ...
        scaled_key_x = self.key[:,0] * ratio_w   ######### i add this for keypointt
        scaled_key_y = self.key[:,1] * ratio_h   ########
        #print(self.key)#################
        #print("ee!@@!@!@!@!", scaled_key_x)#############
        scaled2 = torch.cat([scaled_key_x, scaled_key_y], 0).reshape(-1, 2)#######
        #print(scaled2.shape, self.key.shape)

        #print("SCALEEEDD", scaled2)############



        #box = BoxList(scaled, size, mode='xyxy')
        box = BoxList(scaled, scaled2, size, mode='xyxy') #############
        #print("!!!!!!!!!!RESIZEEEEEEEaa", box.box, box.key)
        ...
        for k, v in self.fields.items():
            ...
            box.fields[k] = v
        #print("!!!!!!!!!!RESIZEEEEEEEaa", box.box, box.key)
        #print("!!!!!!!!!!RESIZEEEEEEEaa", box.convert(self.mode).box, box.convert(self.mode).key)
    ...
    def transpose(self, method):
        ...
        box = BoxList(transpose_box, self.key, self.size, mode='xyxy')##################
    ...
    def to(self, device):
        ...
        box = BoxList(self.box.to(device), self.key, self.size, self.mode)   ####i edit this (self.key)
        #print("QUEST121212112I", box.key, box.box)
        #print("eeeeeee@@@@@@@@@@@@@@@@@@@", self.fields.items())
        
        #print(type(box), len(box), box)
        for k, v in self.fields.items():
            if hasattr(v, 'to'):
                v = v.to(device)
            ...
            box.fields[k] = v
            #print("hahahaha")
        #print(type(box), len(box), box.key, box.box)
        #print(box, "???????????")
